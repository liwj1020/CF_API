     ...
     **-- Global variables:
     ...
     D Idx             s             10i 0
     D EntNbr          s             10i 0
     D HI_Time         s               t   Inz( *HiVal )
     D HI_Date         s               d   DatFmt( *DMY )  Inz( *HiVal )
     D LO_Date         s               d   DatFmt( *DMY )  Inz( *LoVal )
     D JobEntDts       s               z
     D PerBegDts       s               z
     D PerEndDts       s               z
     **-- Period definition:
     ...
     D  PxType6                            LikeDs( Type6 )
      ...
      /Free
        ...
        BegSr  PrcLstEnt;
          ...
          For Idx = 1  To KeyInf.FldNbrRtn;
            ...
            When  KeyInf.KeyFld(Idx) = 402;
              ...
              JobEntDts = %Date( KeyDta.JobEntDat: *CYMD0 ) +
                          %Time( KeyDta.JobEntTim: *HMS0 );
          ...
          If  PrmRcd.CmpSts = '*ALL'  Or
            ...
            If  JobEntDts >= PerBegDts  And  JobEntDts <= PerEndDts;
        ...
        BegSr  *InzSr;
          ...
          If  PrmRcd.Period.NbrElm = 2;

            Select;
            When  PrmRcd.Period.DatBeg = '0010000';
              PrmRcd.Period.DatBeg = %Char( %Date(): *CYMD0 );

            When  PrmRcd.Period.DatBeg = '0020000';
              PrmRcd.Period.DatBeg = %Char( LO_Date: *CYMD0 );
            EndSl;

            Select;
            When  PrmRcd.Period.DatEnd = '0010000';
              PrmRcd.Period.DatEnd = %Char( %Date(): *CYMD0 );

            When  PrmRcd.Period.DatEnd = '0030000';
              PrmRcd.Period.DatEnd = %Char( HI_Date: *CYMD0 );
              PrmRcd.Period.TimEnd = %Char( HI_Time: *HMS0 );
            EndSl;

            PerBegDts = %Date( PrmRcd.Period.DatBeg: *CYMD0 ) +
                        %Time( PrmRcd.Period.TimBeg: *HMS0 );

            PerEndDts = %Date( PrmRcd.Period.DatEnd: *CYMD0 ) +
                        %Time( PrmRcd.Period.TimEnd: *HMS0 );
          EndIf;
