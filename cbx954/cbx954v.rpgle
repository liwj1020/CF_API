     ...
     **
     **  Program description:
     **    Validity checking program for the Remove Group Profile
     **    (RMVGRPPRF) command.
     **
     **
     **  Compile options:
     **    CrtRpgMod Module( CBX954V )
     **              DbgView( *LIST )
     **
     **    CrtPgm    Pgm( CBX954V )
     **              Module( CBX954V )
     ...
     **
     ...
     D  PxFile_q                           LikeDs( File_q )
      ...
      /Free
        ...
        If  ChkObj( PxRmvGrp: '*LIBL': '*USRPRF' ) = *Off;
          SndDiagMsg( 'CPD0006': '0000Group profile does not exist.' );
          SndEscMsg( 'CPF0002': '' );
        EndIf;

        If  %Scan( '*': PxPmtGrp ) = *Zero;

          If  ChkObj( PxPmtGrp: '*LIBL': '*USRPRF' ) = *Off;
            SndDiagMsg( 'CPD0006'
                      : '0000Promote group profile does not exist.'
                      );
