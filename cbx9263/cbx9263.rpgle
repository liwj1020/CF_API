     ...
     **-- Entry parameters:
     D CBX9263         Pr
     D  PxObjNam                   1024a   Const  Varying
     ...
     D  PxRtnVal                     10i 0
      ...
      /Free
        ...
        Else;
          ...
          //-- Check out IFS file:
          CmdStr = 'CHKOUT OBJ(''' + IfsFil + ''')';

          ExcCmd( CmdStr: %Len( CmdStr ));

          //-- Process IFS file:
          FILE_i = open( IfsFil: 'r' );

          If  FILE_i = *Null;
            PxRtnVal = -2;
          Else;

            DoW  fgets( %Addr( RcdBuf ): %Size( RcdBuf ): FILE_i ) <> *Null;
          ...
          //-- Check in IFS file:
          CmdStr = 'CHKIN OBJ(''' + IfsFil + ''')';

          ExcCmd( CmdStr: %Len( CmdStr ));

          //-- Archive IFS file:
          ArcDir = PxPthNam + '/archive';

          If  access( ArcDir: F_OK ) = -1;
            Mode = %BitOr( S_IRWXU: S_IRGRP: S_IROTH );

            rc = Mkdir( ArcDir: Mode );
          EndIf;

          rc = rename( IfsFil: ArcDir + '/' + PxObjNam );
        EndIf;
        ...
        *InLr = *On;
        ...
        BegSr  PrcFilDta;
          ...
          RcdLin = %Str( %Addr( RcdBuf ));

          //-- Process file data here - line-by-line.
          //-- Each line includes a suffixed <new line> x'25' character:
